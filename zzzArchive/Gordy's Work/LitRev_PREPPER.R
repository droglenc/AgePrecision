...
res <- googlesheets4::read_sheet(fn,sheet="Results",na=c("-","")) %>%
  ...
  dplyr::select(-dateStamp,-notes,-tech,-USE)
study <- googlesheets4::read_sheet(fn,sheet="Study",na=c("-","")) %>%
  dplyr::select(-studysite,-(notes:OrigFile))


### Several variables will eventually be converted to factors and the number of
###   levels will be reduced. These vectors streamline that process.
unique(fish$class)
classes <- data.frame(
  old=c("Actinopteri","Elasmobranchii","Holocephali","Petromyzonti"),
  new=c("Actinopteri","Elasmobranchii","other","other")
  )
# Use this to set levels later ... levels=unique(classes$new)
...
res2 <- res %>%
  dplyr::mutate(
    ...
    bias_usedplot=ifelse(stringr::str_detect(biasmethod,"agebiasplot"),
                         "yes","no"),
    bias_usedplot=factor(bias_usedplot,levels=lvls_yn),
    bias_usedregression=ifelse(stringr::str_detect(biasmethod,"regression"),
                               "yes","no"),
    bias_usedregression=factor(bias_usedregression,levels=lvls_yn),
    bias_usedsymtest=ifelse(stringr::str_detect(biasmethod,"symmetry"),
                            "yes","no"),
    bias_usedsymtest=factor(bias_usedsymtest,levels=lvls_yn),
    bias_usedttest=ifelse(stringr::str_detect(biasmethod,"t-test"),"yes","no"),
    bias_usedttest=factor(bias_usedttest,levels=lvls_yn),
    bias_usedother=unlist(lapply(strsplit(biasmethod,", "),FUN=function(x)
      ifelse(all(is.na(x)),NA_character_,
             ifelse(!all(sjmisc::str_contains(c("agebiasplot","regression",
                                                "t-tests","symmetry test"),x)),
                    "yes","no")))),
    bias_usedother=factor(bias_usedother,levels=lvls_yn),
...
LR <- tmp %>%
  ...
  dplyr::select(
    ...
    checkbias,bias_usedplot,bias_usedregression,bias_usedsymtest,bias_usedttest,
    bias_usedother,biasmethod,biaspresent,
