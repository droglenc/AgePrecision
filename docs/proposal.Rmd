...
Accuracy is related to estimating the true age of the fish. Accuracy is related to validation which was extensively discussed by @campana_accuracy_2001 and won’t be discussed further here.
...
However, note that when $R$ = 2 then $D_{j}$ = $APE_{j}$ because $CV_{j}=\sqrt{2}APE_{j}$ when $R$ = 2. This metric will be labelled as $AD$ when averaged across all $n$ fish.
...
and a summary across all $n$ fish may look like the following (note that $ACV2$, $APE2$, and $AD$ are not shown because $R$ = 2).
...
* Study ID (Author name(s) appended with publication year ... e.g., Lepaketal_2017)
* Species name
* Study site
* Publication year
* Structure (e.g., scales, otoliths, finrays, vetebrae, cleithra)
* Structure2 (e.g., dorsal, pectoral, pelvic)
* Process (e.g., whole, sectioned, crackburn)
* Number of readers ($R$)
* Number of samples ($n$)
* Age range (minimum and maximum age among all readers)
...
* Number of readers ($R$)
* Number of samples ($n$)
* Age range (minimum, 10th percentile, 90th percentile, and maximum mean age among all readers)
...
* AAPE2
* AD
* Type of relationship between precision metrics and age
    * Fit linear and quadratic models with and without modeling heteroscedasticity (i.e., generalized least-squares models)
        * Determine if heteroscedasticy needed to be modeled
        * Determine if the quadratic term was needed
        * Determine if a linear relatinship existed

Raw data files will be stored as CSV files with a corresponding meta-data text file (an example has been created for Lepak_etal_2017). The data will be initially processed with `agePrecision()` from the `FSA` package. Information above will be extracted from this result using `precisionData()` written for this project. The resulting object will be saved as an RDS file for post-processing to produce a data.frame (and ultimately a flat database) of the items above.
