...
nm <- "spiegel_precision_2010"       ## Name of study (actually name of file)
...
df1 <- df %>%
  filter(species=="Highfin Carpsucker",structure=="Fin Rays") %>%
...
df1 <- df %>%
  filter(species=="Highfin Carpsucker",structure=="Scales") %>%
  select(contains("read")) %>%
  filter(complete.cases(.))
...
df1 <- df %>%
  filter(species=="Quillback Carpsucker",structure=="Fin Rays") %>%
  select(contains("read")) %>%
  filter(complete.cases(.))
...
df1 <- df %>%
  filter(species=="Quillback Carpsucker",structure=="Scales") %>%
  select(contains("read")) %>%
  filter(complete.cases(.))
...
df1 <- df %>%
  filter(species=="River Carpsucker",structure=="Fin Rays") %>%
  select(contains("read")) %>%
  filter(complete.cases(.))
...
df1 <- df %>%
  filter(species=="River Carpsucker",structure=="Scales") %>%
  select(contains("read")) %>%
  filter(complete.cases(.))
