...
nm <- "glass_pectoral_2011"       ## Name of study (actually name of file)
...
ap1 <- agePrecision(~finrays_R1+finrays_R2,data=df1)
...
df1 <- df %>%
  select(contains("branchio")) %>%
  filter(complete.cases(.))

ap1 <- agePrecision(~branchio_R1+branchio_R2,data=df1)
pt1SD <- precisionData(ap1,studyID=nm,species=species,
                       structure=strux,structure2=strux2,process=proc,
                       type=atype,var="SD")
plot(pt1SD)
