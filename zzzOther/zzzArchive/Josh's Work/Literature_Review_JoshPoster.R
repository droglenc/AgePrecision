...
dfoto <- filter(df,structure=="otoliths")
dfoto <- filter(dfoto,process=="sectioned" | process=="whole")

dfbw <- filter(df,type=="between" | type=="within")
dfb <- filter(df,type=="between")
dfw <- filter(df,type=="within")
dfob <- filter(dfoto,type=="between")
dfow <- filter(dfoto,type=="within")

dftc <- filter(df,class=="Actinopteri" | class=="Elasmobranchii")

dfacv <- filter(df,ACVcheck=="yes")
dfacv1 <- filter(df,ACV>0)

dfamod <- filter(df,ACVmodcheck=="yes")
dfamod1 <- filter(df,ACVmod>0)
dftype <- filter(dfamod1,type=="between" | type=="within")
dfamod1class <- filter(dfamod1,class=="Actinopteri" | class=="Elasmobranchii") 
